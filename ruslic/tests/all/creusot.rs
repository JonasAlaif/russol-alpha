...
#[trusted]
impl<T> WellFounded for NodeAlt<T> {}
...
mod private {
    ...
    #[ensures(*result == p.0)]
    #[ensures(^result == (^p).0)]
    #[ensures(if ^result == 10_u16 { is_ten(^p) } else { true })]
    pub fn bar(p: &mut Percentage) -> &mut u16 {
        &mut p.0
    }

    #[logic] pub fn is_ten(p: Percentage) -> bool { p.0 == 10_u16 }
}
...
impl Node<i32> {
    ...
    // #[ensures(if result.len_gt0() == 0 { result.elems() == logic::Set::EMPTY } else { result.elems() == logic::Set::EMPTY.insert(0) })]
    // #[ensures(^self == ^result)]
    ...
    #[ensures((^i).elems() == match **i {
        Node::Cons { elem, .. } => (^result.1).elems().insert(elem).insert(^result.0),
        _ => logic::Set::EMPTY,
    })]
...
#[trusted]
impl<T> WellFounded for Tree<T> {}
impl<T> Tree<T> {
    #[logic]
    #[variant(self)]
    #[ensures(result >= 0)]
    fn size(self) -> logic::Int {
        match self {
            Tree::Leaf => 0,
    ...
    fn elems(self) -> logic::Set<T> {
        match self {
            Tree::Leaf => logic::Set::EMPTY,
            Tree::Node { f, left, right } => left.elems().union(right.elems()).insert(f),
        }
    }
...
impl Tree<i32> {
    ...
    //         Tree::Leaf => true,
    //         Tree::Node { f, left, right } => {
    //             (if let box Tree::Node { f: f_l, .. } = left { f_l <= f } else { true }) &&
    //             (if let box Tree::Node { f: f_r, .. } = right { f <= f_r } else { true }) &&
    ...
    #[ensures(match (*self, ^self) {
        (Tree::Leaf, _) => (^self).elems() == logic::Set::EMPTY.insert(v),
        (Tree::Node { f, left, right },
         Tree::Node { left: l, right: r, .. }) => {
            if v < f {
                l.elems() == left.elems().insert(v)
            } else if f < v {
                r.elems() == right.elems().insert(v)
            } else { true }
        }
        _ => false
    })]
    #[ensures((^self).elems() == self.elems().insert(v))]
...
#[ensures(match (i, result) {
    (OptionCustom::Left { left }, OptionCustom::Right { right }) => *left+1_i16 == right,
    (OptionCustom::Right { right }, OptionCustom::Left { left }) => left == *right+1_i16,
    _ => false,
})]
...
#[requires(^x == 1_i32)]
#[ensures(*x == 999_i32)] // as hard as `false`
...
#[requires(match x.z { Enum::V2 { f6, .. } => f6 >= i.f4 && @i.f4 >= 0, _ => false })]
#[ensures(match x.z {
    Enum::V1 { .. } => true,
...
#[trusted]
impl<T> WellFounded for ListSLL<T> {}
struct NodeSLL<T> { elem: T, next: Link<T> }
#[trusted]
impl<T> WellFounded for NodeSLL<T> {}

impl<T> NodeSLL<T> {
    #[logic]
    #[trusted]
    #[ensures(result == (1 + (match self.next {
        None => 0,
        Some(n) => n.len(),
    })))]
    #[ensures(result >= 1)]
    pub fn len(self) -> logic::Int {
        1 + (match self.next {
            None => 0,
            Some(n) => n.len(),
        })
...
impl<T> ListSLL<T> {
    ...
    #[logic]
    #[trusted]
    #[ensures(result == (match self.head {
        None => 0,
        Some(n) => n.len(),
    }))]
...
impl<'a, T> Iter<'a, T> {
    #[ensures(if self.len() == 0 { (^self).len() == 0 && is_none(result) }
        else { (^self).len() == self.len()-1 && is_some(result) }
    )]
...
impl NodeSLL<i32> {
    #[logic]
    #[trusted]
    #[ensures(result == (match self.next {
        None => self.elem,
        Some(next) => (self.elem + next.sum()),
    }))]
    fn sum(self) -> i32 {
        match self.next {
            None => self.elem,
...
// impl<T> Node<T> {
//     #[trusted]
//     #[ensures(result.elems() == self.elems().union(other.elems()))]
...
#[trusted]
impl<T> WellFounded for ListRT<T> {}
impl<T> ListRT<T> {
    #[logic]
    #[trusted]
    #[ensures(result == (match self {
        ListRT::Nil => logic::Set::EMPTY,
        ListRT::Cons(box (hd, tl)) => tl.elems().insert(hd),
    }))]
    fn elems(self) -> logic::Set<T> {
        match self {
            ListRT::Nil => logic::Set::EMPTY,
...
//     #[logic]
//     fn elems_tree(self) -> logic::Set<T> {
//         match self {
//             ListRT::Nil => logic::Set::EMPTY,
//             ListRT::Cons(box (hd, tl)) => hd.elems().union(tl.elems_tree()),
//         }
//     }
// }
...
impl<T> RoseTree<T> {
    ...
    // fn elems(self) -> logic::Set<T> {
    //     match self {
    //         RoseTree::Nil => logic::Set::EMPTY,
    //         RoseTree::Cons { elem, next } => next.elems_tree().insert(elem),
    //     }
    // }
...
impl<T> ListRT<ListRT<T>> {
    ...
    #[ensures(result == (match self {
        ListRT::Nil => logic::Set::EMPTY,
        ListRT::Cons(box (hd, tl)) => hd.elems().union(tl.elems_list()),
    }))]
    fn elems_list(self) -> logic::Set<T> {
        match self {
            ListRT::Nil => logic::Set::EMPTY,
            ListRT::Cons(box (hd, tl)) => hd.elems().union(tl.elems_list()),
        }
...
impl Node<u16> {
    ...
    #[trusted]
    #[ensures(result == (match self {
        Node::Cons { elem, next } => pearlite!{@elem} + next.sum(),
        Node::Nil => 0,
    }))]
    fn sum(self) -> logic::Int {
        match self {
            Node::Cons { elem, next } => pearlite!{@elem} + next.sum(),
            Node::Nil => 0,
        }
...
#[trusted]
#[ensures(result == if a + b >= 256_u16 { a + b - 256_u16 } else { a + b })]
pub fn wrapping_add(a: u16, b: u16) -> u16 {
    a.wrapping_add(b)
}
#[ensures(result == a + b || result == a + b - 256_u16)]
pub fn test_u8_wrapping_add(a: u16, b: u16) -> u16 {
  let result = wrapping_add(b as u16, a);
  result
}
...
#[requires(x.0 == 123_i32 && x.1 == 42_i32)]
#[ensures(result.0 == 42_i32 && result.1 == 123_i32)]
fn test_tuple_field(x: (i32, i32)) -> (i32, i32) {
  (42 as i32, 123 as i32)
}
#[requires(x.a == 123_i32 && x.b == 42_i32)]
#[ensures(result.a == 42_i32 && result.b == 123_i32)]
fn test_struct_field(x: S) -> S {
  S { a: 42 as i32, b: 123 as i32 }
}

struct Number<A, B, C> { a: A, b: B, c: C }
#[requires(-10_000_i32 < arg.b && arg.b < 10_000_i32)]
#[ensures((^arg).b == arg.b - 1000_i32)]
fn test1<A, B>(arg: &mut Number<A, i32, B>) {
  let de = arg.b;
  arg.b = (de - 1000) as i32
}
#[requires(-10_000_i32 < arg.b.b && arg.b.b < 10_000_i32)]
#[ensures((^arg).b.b == arg.b.b - 1000_i32)]
...
// }
#[requires(x == -42_i32)]
#[ensures(match result { Some(k) => k == -42_i32, _ => false })]
